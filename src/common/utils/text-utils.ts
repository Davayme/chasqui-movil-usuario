...
/**
 * Verifica si un nombre o apellido está presente en el texto extraído del documento
 * @param searchText - Texto a buscar (nombre o apellido)
 * @param extractedText - Texto completo extraído del documento
 * @returns boolean - Si el texto fue encontrado
 */
export const isTextInDocument = (searchText: string, extractedText: string): boolean => {
  ...
  // Dividir el texto de búsqueda en palabras para buscar cada una
  const searchWords = normalizedSearch.split(' ').filter(word => word.length > 1);
  
  // Verificar que todas las palabras del nombre/apellido estén presentes
  return searchWords.every(word => normalizedExtracted.includes(word));
};
